...
<!--
  ...
  - kind="tabs":     ToggleGroup type="single" — pick one panel section
                     OR none (deselect = panel closed). aria-pressed
                     on items, arrow-key nav.
  ...
  ┌──┐
  │⚙ │  TABS — solid bg + 3px blue left-edge accent on active
  │🔍│
  │◉ │
  ├──┤
  │⊞ │  RADIO — solid bg, no left-edge accent (subtler than tabs)
  │⊟ │
  ├──┤
  ...
  data + handlers via the `sections` prop.
-->
<script lang="ts">
  ...
  type SectionKind = "tabs" | "radio" | "toggles" | "momentary";

  interface StripButton {
    /** Required for "tabs" / "radio" sections — identifies this
     * button as a ToggleGroup.Item value. Ignored for "toggles" /
     * "momentary". */
    value?: string;
    icon: Component<{ class?: string }>;
    title: string;
    /** Visual active state. For tabs / radio: matches the section's
  ...
  /**
   * Bridge bits-ui `ToggleGroup.Root onValueChange` to our
   * button-level `onClick` handlers. For single-mode groups, bits-ui
   * passes the newly-selected value on select and an empty string on
   * deselect (in single mode). We dispatch to the corresponding
   * button's onClick.
   */
  function handleGroupChange(buttons: StripButton[], v: string) {
    if (v === "") {
      // Deselected — fire the previously-active button's onClick
      // (which is responsible for clearing). Used by the "tabs" kind
      // to close the panel when the user clicks the active tab.
...
>
  {#each sections as section, i (section.key)}
    ...
    {#if section.kind === "tabs" || section.kind === "radio"}
      {@const activeValue = section.buttons.find((b) => b.active)?.value ?? ""}
      <ToggleGroup.Root
        type="single"
        orientation="vertical"
        value={activeValue}
        onValueChange={(v) => handleGroupChange(section.buttons, v)}
        class="flex flex-col"
      >
        {#each section.buttons as button (button.value ?? button.title)}
          {@const Icon = button.icon}
          {@const isActive = button.active === true}
          {@const isTab = section.kind === "tabs"}
          <ToggleGroup.Item
            value={button.value ?? ""}
            title={button.title}
            aria-label={button.title}
            class="h-12 w-12 flex flex-col items-center justify-center transition focus-visible:outline-2 outline-blue-600 -outline-offset-1 {isTab
              ? 'border-l-[3px]'
              : ''} {isActive
              ? isTab
